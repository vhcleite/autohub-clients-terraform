...
variable "lambda_function_name_http" {
  description = "Nome base para a função Lambda da API HTTP de Vendas"
  type        = string
  default     = "AutoHubSalesApiHttp"
}
...
variable "lambda_timeout_http" {
  ...
  type        = number
  default     = 60 # API GW tem limite de 29s, mas a função pode rodar mais
}

# --- Variáveis para Lambda SQS ---
variable "lambda_function_name_sqs" {
  description = "Nome base para a função Lambda do Listener SQS de Vendas"
  type        = string
  default     = "AutoHubSalesApiSqs"
}
...
variable "lambda_jar_path" {
  description = "OBRIGATÓRIO: Caminho para o JAR da API de Vendas (Fat JAR)"
  type        = string
}
